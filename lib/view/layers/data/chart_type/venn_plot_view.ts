...
export class VennPlotView extends DataLayer {
  ...
  protected _createDatapoints3Circles() {
    ...
    const centers: { x: number; y: number }[] = [
      { x: cx - offsetX * 0.7, y: cy + offsetY * 0.7},
      { x: cx + offsetX * 0.7, y: cy + offsetY * 0.7},
      { x: cx, y: cy - offsetY * 0.9},
    ];

    const circle1 = { center: { x: cx - offsetX * 0.7, y: cy + offsetY * 0.7 }, radius: radius, name: "A" };
    const circle2 = { center: { x: cx + offsetX * 0.7, y: cy + offsetY * 0.7 }, radius: radius, name: "B" };
    const circle3 = { center: { x: cx, y: cy - offsetY * 0.9 }, radius: radius, name: "C" };
    ...
    const [p1, p2] = this.getIntersections(
      { center: { x: cx - offsetX * 0.7, y: cy + offsetY * 0.7 }, radius: radius, name: 'A' },
      { center: { x: cx + offsetX * 0.7, y: cy + offsetY * 0.7 }, radius: radius, name: 'B' }
    );

    const [p3, p4] = this.getIntersections(
      { center: { x: cx - offsetX * 0.7, y: cy + offsetY * 0.7 }, radius: radius, name: 'A' },
      { center: { x: cx, y: cy - offsetY * 0.9 }, radius: radius, name: 'C' }
    );
