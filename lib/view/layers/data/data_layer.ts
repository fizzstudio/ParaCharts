...
export abstract class DataLayer extends PlotLayer {
  ...
  protected _beginDatapointLayout() {
    this.paraview.paraState.thresholds.push(new Threshold(this.paraview, 'horiz', 59000, 'Average'))
  ...
  removeDatapointPopup(dpView: DatapointView) {
    ...
    this.paraview.paraState.selectPopups.splice(this.paraview.paraState.selectPopups.findIndex(p => p.id === dpView.id), 1)
    dpView.popup?.remove();
    this.paraview.requestUpdate()
  }
