...
/*
  ...
  get datapointViews() {
    return super.datapointViews as HeatmapDatapointView[];
  }
  get grid(){
    return this._grid;
  }
  ...
  protected _newDatapointView(seriesView: XYSeriesView) {
    return new HeatmapDatapointView(seriesView);
  }

    protected _createComponents() {
        this._resolution = this.settings.resolution ?? 25;
        const xs: string[] = [];
        ...
        for (const [col, i] of enumerate(this.paraview.store.model!.series)) {
            ...
            for (let i = 0; i < this._resolution ** 2; i++) {
                const heatmapTile = new HeatmapTileView(this, seriesView.seriesKey);
  ...
  protected _generateHeatmap(): Array<Array<number>> {
    ...
    let yMax: number = this.axisInfo?.yLabelInfo.max!;
    let xMax: number = this.axisInfo?.xLabelInfo.max!;
    let yMin: number = this.axisInfo?.yLabelInfo.min!;
    let xMin: number = this.axisInfo?.xLabelInfo.min!;
...
*/
  ...
  moveLeft() {
    const leaf = this._chartLandingView.focusLeaf;
    if (leaf instanceof HeatmapTileView) {
      if (!leaf.prev || leaf.prev instanceof HeatmapDatapointView) {
        //leaf.blur(false)
        //this._eventActionManager!.dispatch('series_endpoint_reached');
      }
      else {
  ...
  moveUp() {
    ...
    if (leaf instanceof HeatmapTileView) {
      ...
      if (!target || target instanceof HeatmapDatapointView) {
        //leaf.blur(false)
        //this._eventActionManager!.dispatch('series_endpoint_reached');
      }
      else {
...
export class HeatmapTileView extends DataView {
  ...
  protected computeLocation(){
    ...
    this._x = (this.index - length) % this.chart.resolution * this._width
