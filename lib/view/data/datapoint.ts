...
export class DatapointView extends DataView {
  ...
  constructor(paraState: ParaState, seriesView: SeriesView) {
    super(paraState, seriesView.chart, seriesView.series.key);
  }
  ...
  get classInfo(): ClassInfo {
    ...
    return {
      datapoint: true,
      visited: this._paraState.isVisited(this.seriesKey, index),
      selected: this._paraState.isSelected(this.seriesKey, index),
      highlighted: this._paraState.isHighlighted(this.seriesKey, index)
    };
  }
  ...
  protected get symbolScale() {
    if (this._paraState.isVisited(this.seriesKey, this.index)) {
      return this._paraState.settings.chart.symbolHighlightScale * this._baseSymbolScale;
    } else if (this._paraState.isHighlighted(this.seriesKey, this.index)) {
      return 1; //this._paraState.settings.chart.symbolHighlightScale;
    } else {
  ...
  protected get _symbolColor() {
    ...
    return this._paraState.isVisited(this.seriesKey, this.index) ? -1 as number :
  ...
  protected _contentUpdateSymbol() {
    if (this._symbol) {
      ...
      this._symbol.hidden = !this._paraState.settings.chart.isDrawSymbols;
    }
  }
  ...
  addDatapointPopup(text?: string, xInput?: number, yInput?: number) {
    ...
    if (['bar', 'column', 'waterfall'].includes(this._paraState.type)) {
      x = this.x + this.width / 2
      if (this._paraState.settings.popup.activation == "onHover") {
        pointerControlled = true;
      }
    }
    if (['waterfall'].includes(this._paraState.type)) {
    ...
    let popup = new Popup(this._paraState, this.paraview,
      {
        ...
        color: color,
        points: [this],
        rotationExempt: this._paraState.type == 'bar' ? false : true,
        angle: this._paraState.type == 'bar' ? -90 : 0,
        pointerControlled
      },
      {
  ...
  movePopupAction() {
    if (this._popup) {
      if (['column', 'waterfall', 'pie', 'donut'].includes(this._paraState.type)) {
        this.addDatapointPopup(undefined, this._paraState.pointerCoords.x, this._paraState.pointerCoords.y)
        this._popup.horizShift = this._paraState.pointerCoords.x - (this._popup.grid.x + this._popup.grid.width / 2)
      }
      else if (this._paraState.type == 'bar') {
        this.addDatapointPopup(undefined, this._paraState.pointerCoords.y, this.chart.height - this._paraState.pointerCoords.x)
      }
    }
  }
